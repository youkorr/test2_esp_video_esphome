...
void ESPVideoComponent::setup() {
  ...
  // Note: L'initialisation matérielle (I2C, LDO, XCLK, CSI, ISP) est effectuée
  // par les composants qui l'utilisent:
  // - i2c: Gère la communication I2C
  // - esp_ldo: Gère l'alimentation MIPI
  // - mipi_dsi_cam: Initialise le capteur, XCLK, CSI et ISP
  //
  // Ce composant se contente d'activer les flags de compilation nécessaires
  // et de fournir les en-têtes et bibliothèques pour ESP-Video.

  ESP_LOGI(TAG, "Note: Configuration matérielle gérée par:");
  ESP_LOGI(TAG, "  - i2c: Communication avec le capteur");
  ESP_LOGI(TAG, "  - esp_ldo: Alimentation MIPI");
  ESP_LOGI(TAG, "  - mipi_dsi_cam: Initialisation capteur/CSI/ISP");

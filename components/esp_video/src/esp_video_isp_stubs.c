                                                           ...
                                                           const struct v4l2_format *format)
{
    if (state && state->bypass_isp) {
        ESP_LOGW(TAG, "ISP pipeline disabled; bypassing processing");
        return ESP_OK;
    }
    ...
    ESP_LOGE(TAG, "ISP pipeline not available (pix=0x%08" PRIx32 ")", pix_format);
...
{
    ESP_LOGE(TAG, "ISP video device not available in this build");
    return ESP_ERR_NOT_SUPPORTED;
}

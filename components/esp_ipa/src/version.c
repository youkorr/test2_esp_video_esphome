...
const esp_ipa_config_t *esp_ipa_pipeline_get_config(const char *cam_name)
{
    ...
    if (cam_name) {
        if (strcmp(cam_name, "OV5647") == 0 || strcmp(cam_name, "ov5647") == 0) {
            ...
            // SC202CS: Désactiver CCM (cause teinte verte), utiliser RGB gains uniquement
            ESP_LOGI(TAG, "📸 IPA config for %s: AWB+Denoise+Sharpen+Gamma (4 algos, CCM disabled - use RGB gains)", cam_name);
            return &ipa_config_ov5647;

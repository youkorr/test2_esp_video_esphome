#!/usr/bin/env python3
"""
ESP-DL Model Package Tool
Adapted from Waveshare ESP32-P4-WIFI6-Touch-LCD-7B

Packs multiple .espdl model files into a single binary file for easier deployment.
"""
...
def pack_models(model_path_or_dir, out_file="models.espdl"):
    ...
    for model_file in model_files:
        ...
        print(f"  ✓ {model_file.name} ({len(model_bins[-1])} bytes)")
    ...
    print(f"\n✅ Packed {model_num} models into: {out_file} ({len(out_bin)} bytes)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--model_path", type=str, nargs="+",
        help="the path of model files or directory"
    )
    parser.add_argument(
        "-o",
        "--out_file",
        type=str,
        default="human_face_detect.espdl",
        help="the path of output packed binary file",
    )
    args = parser.parse_args()

    print("ESP-DL Model Packaging Tool")
    print("=" * 50)

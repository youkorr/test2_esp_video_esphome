...
void SimpleVideoPlayer::stop() {
  ...
  if (this->frame_count_ > 0) {
    ...
    if (this->frames_dropped_ > 0) {
      ESP_LOGI(TAG, "Playback stats: %lu frames played, %lu dropped (%.1f%% drop rate)",
               (unsigned long)this->frame_count_, (unsigned long)this->frames_dropped_, drop_rate);
      ESP_LOGI(TAG, "Frame drops indicate system overload - consider lowering FPS or resolution");
    } else {

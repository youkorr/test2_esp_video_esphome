...
#include "esp_audio_dec.h"    // AAC audio decoder
#include "esp_aac_dec.h"      // AAC decoder implementation
#include "esp_audio_types.h"  // Audio codec types
...
void SimpleVideoPlayer::setup() {
  ...
  if (this->rgb_buffer_ == nullptr) {
    ESP_LOGE(TAG, "Failed to allocate RGB front buffer (%u bytes)", this->rgb_buffer_size_);
    this->mark_failed();
    return;
  }
  ...
  // Create semaphore for decode task signaling
  this->decode_sem_ = xSemaphoreCreateBinary();
  if (this->decode_sem_ == nullptr) {
    ESP_LOGE(TAG, "Failed to create decode semaphore");
  }

  // Create dedicated decode task (like avi_player) for async decode
  // This prevents timer callback from blocking, improving FPS performance
  ...
  if (task_created != pdPASS) {
    ...
    ESP_LOGI(TAG, "Decode task created (async decode like avi_player)");
  }
...
bool SimpleVideoPlayer::open_video_file_() {
  ...
  if (this->is_http_source_) {
    ...
    // Use system default buffering (typically 4KB in internal RAM)
    // This is faster than large PSRAM buffers due to lower access latency
    // For best performance, use preload_to_memory: true to load entire file to PSRAM
    ESP_LOGI(TAG, "Using system default file buffering (internal RAM)");
...
bool SimpleVideoPlayer::decode_mjpeg_frame_() {
  ...
  esp_err_t ret = jpeg_decoder_process(this->jpeg_decoder_, &decode_cfg,
                                        ...
                                        write_buffer, this->rgb_buffer_size_,
                                        &out_size);
...
bool SimpleVideoPlayer::parse_esds_(uint32_t size) {
  ...
  while (this->cached_ftell_() < end_pos) {
    ...
    if (tag == 0x05) {  // DecoderSpecificInfo
      // This is the AAC config for audio decoding
      this->audio_config_.resize(len);
      this->cached_fread_(this->audio_config_.data(), 1, len);
      this->has_audio_ = true;  // Enable audio decoding
      ESP_LOGI(TAG, "Found AAC config: %d bytes (audio enabled)", len);
      break;
    } else {
...
bool SimpleVideoPlayer::init_aac_decoder_() {
  ...
  esp_aac_dec_cfg_t aac_cfg = {
    .sample_rate = (int32_t)this->audio_sample_rate_,
    .channel = this->audio_channels_,
    .bits_per_sample = 16,  // Output PCM is always 16-bit
    .no_adts_header = true,  // MP4 AAC has no ADTS header
    .aac_plus_enable = true,
  };

  // Open decoder
  esp_audio_err_t ret = esp_aac_dec_open(&aac_cfg, sizeof(aac_cfg), &this->aac_decoder_);
  ...
  ESP_LOGI(TAG, "AAC decoder initialized: %u Hz, %u channels",
...
bool SimpleVideoPlayer::decode_audio_frame_() {
  ...
  esp_audio_dec_in_raw_t in_frame = {
    ...
    .len = (uint32_t)this->audio_input_size_,
    .consumed = 0,
  ...
  esp_audio_err_t ret = esp_aac_dec_decode(this->aac_decoder_, &in_frame, &out_frame, &dec_info);
  ...
  if (out_frame.decoded_size > 0) {
    ...
    if (bytes_written == 0) {
      ESP_LOGV(TAG, "Speaker buffer full, audio dropped");
    }
  }
...
void SimpleVideoPlayer::process_audio_() {
  if (!this->has_audio_ || this->speaker_ == nullptr || !this->aac_decoder_ready_) {
    return;
  }

  // Process audio samples to keep in sync with video
  // Simple implementation: process audio samples that correspond to current video position
  while (this->current_audio_sample_ < this->audio_samples_.size()) {
    ...
    // Audio timestamp should be <= current video time
    if (this->current_video_sample_ > 0 && this->current_video_sample_ <= this->video_samples_.size()) {
      uint32_t video_time_ms = this->video_samples_[this->current_video_sample_ - 1].timestamp_ms;
      uint32_t audio_time_ms = sample.timestamp_ms;

      // If audio is ahead of video, stop processing
      if (audio_time_ms > video_time_ms + 100) {  // 100ms threshold
        break;
      }
    }
...
bool SimpleVideoPlayer::decode_h264_frame_() {
  ...
  if (out_frame.out_size > 0 && out_frame.outbuf != nullptr) {
    ...
    this->convert_i420_to_rgb565_(out_frame.outbuf, write_buffer,
                                   this->actual_width_, this->actual_height_);
...
bool SimpleVideoPlayer::parse_mkv_track_entry_(uint64_t size) {
  ...
  if (track_type == 1 && (codec_id == "V_MPEG4/ISO/AVC" || codec_id == "V_AVC")) {
    ...
    // Audio track with AAC codec
    this->mkv_audio_track_ = track_number;
    this->has_audio_ = true;  // Enable audio decoding
    ESP_LOGI(TAG, "Found AAC audio track %u (audio enabled)", track_number);
  }
...
void SimpleVideoPlayer::play() {
  ...
  if (this->input_buffer_ == nullptr || this->rgb_buffer_ == nullptr) {
    ...
    // Re-initialize AAC audio decoder if needed (for MP4/MKV)
...
void SimpleVideoPlayer::stop() {
  ...
  // Stop decode task (like avi_player cleanup)
  if (this->decode_task_handle_ != nullptr) {
    // Signal task to exit
    this->decode_task_running_ = false;

    // Wake task one last time so it sees the flag and exits
    if (this->decode_sem_ != nullptr) {
      xSemaphoreGive(this->decode_sem_);
    }

    // Wait briefly for task to exit gracefully
    vTaskDelay(pdMS_TO_TICKS(50));

    // Task will delete itself via vTaskDelete(nullptr)
  ...
  // Free RGB output buffers
  if (this->rgb_buffer_ != nullptr) {
    total_freed += this->rgb_buffer_size_;
    heap_caps_free(this->rgb_buffer_);
    this->rgb_buffer_ = nullptr;
    ESP_LOGD(TAG, "  Freed rgb_buffer_ (front): %zu bytes", this->rgb_buffer_size_);
  }
  ...
  // Free audio buffers (AAC decoder)
...
// NEW ARCHITECTURE: Timer now just signals decode task (non-blocking, fast!)
// Decode task does heavy work (read + decode) asynchronously like avi_player
...
void SimpleVideoPlayer::esp_timer_cb_(void *arg) {
  ...
  // Signal decode task to wake up and process next frame (non-blocking!)
  if (player->decode_sem_ != nullptr) {
    xSemaphoreGive(player->decode_sem_);
  }

  // FPS measurement for performance monitoring
  ...
  if (callback_count++ % 30 == 0 && fps_measure_start > 0) {
    ...
    ESP_LOGI(TAG, "Performance: %.2f FPS (avg interval %.1fms) | target: %.0f FPS (%.0fms interval) [async decode]",
  ...
  // That's it! Decode task does the heavy work. Timer stays fast and precise.
}

// Dedicated decode task (runs asynchronously like avi_player)
// This prevents the timer callback from blocking, allowing target FPS to be achieved
void SimpleVideoPlayer::decode_task_(void *arg) {
  SimpleVideoPlayer *player = static_cast<SimpleVideoPlayer *>(arg);
  ESP_LOGI(TAG, "Decode task started");

  while (player->decode_task_running_) {
    // Wait for signal from timer callback (blocking)
    if (xSemaphoreTake(player->decode_sem_, portMAX_DELAY) != pdTRUE) {
      continue;
    }
    ...
    // Check if previous frame is still pending (not yet displayed by loop())
    if (player->frame_ready_) {
      player->frames_dropped_++;
      static uint32_t last_drop_warning = 0;
      uint32_t now = esp_timer_get_time() / 1000;
      if (now - last_drop_warning > 1000) {
        ESP_LOGI(TAG, "Frame drops: %lu frames (LVGL display slow)", (unsigned long)player->frames_dropped_);
    ...
    // Handle first frame rotation reinitialization
    if (got_frame && player->rotation_ != 0 && player->rotate_handle_ != nullptr && !player->rotation_initialized_) {
      ...
      if (ret != ESP_IMGFX_ERR_OK) {
        ESP_LOGE(TAG, "Failed to reinitialize rotation: %d", ret);
        player->rotate_handle_ = nullptr;
      } else {
        ESP_LOGI(TAG, "Rotation reinitialized: %dx%d -> %d degrees",
                 player->actual_width_, player->actual_height_, player->rotation_);
        player->rotation_initialized_ = true;

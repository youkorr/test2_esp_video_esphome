...
/* --------------------------------------------------------------------------
 *  Mode 640x480 @ 30fps (CROP centré, PAS de binning)
 *
 *  Le SC202CS n'a PAS de binning hardware !
 *  Ce mode utilise un CROP centré sur le capteur 1600x1200.
 *
 *  Calculs (basés sur le mode 1280x720 fonctionnel):
 *  - Capteur plein : 1600×1200
 *  - Zone de crop centrée pour 640x480 + 8 pixels de blanking:
 *      startX = 476    = 0x01DC  (centre - 324)
 *      endX   = 1123   = 0x0463  (centre + 323)
 *      startY = 356    = 0x0164  (centre - 244)
 *      endY   = 843    = 0x034B  (centre + 243)
 *  - Zone = 648×488, Sortie = 640×480 avec offset 4
 * --------------------------------------------------------------------------*/
...
static const sc202cs_reginfo_t init_reglist_MIPI_1lane_raw8_640x480_30fps[] = {
    ...
    /* ROI centré 648x488 sur capteur 1600x1200 */
...
/* --------------------------------------------------------------------------
 *  Descripteur de format esp_video pour le mode 640x480
 * --------------------------------------------------------------------------*/

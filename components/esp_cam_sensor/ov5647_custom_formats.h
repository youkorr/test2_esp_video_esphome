...
// 800x640 clock rates (native sensor resolution)
...
// ============================================================================
// Configuration 4 : 800x640 @ 50fps RAW8 (STRIDE FIX - centered crop, offset=0)
// ============================================================================
// Native OV5647 resolution. Fixed vs native driver (ov5647_settings.h):
//   - Crop centered at X=234 (native uses X=500 which is off-center)
//   - Horizontal offset = 0 (native uses offset=8 which causes stride artifacts)
// These changes eliminate the stride/alignment issues in the ISP/DMA pipeline.

static const ov5647_reginfo_t ov5647_input_24M_MIPI_2lane_raw8_800x640_50fps_fixed[] = {
    // RAW8 mode configuration
    {0x3034, OV5647_8BIT_MODE},  // 8-bit RAW8 format
    ...
    // STRIDE FIX: Crop window CENTERED on sensor (not offset like native)
    // Native driver uses X_start=500 which is off-center and causes stride artifacts
    // Sensor: 2592x1944, Crop: 2124x1954 pixels
    // X centered: (2592 - 2124) / 2 = 234
    {0x3800, (234 >> 8) & 0x0F},   // X address start high
    {0x3801, 234 & 0xFF},          // X address start low
    ...
    // STRIDE FIX: Timing offset = 0 (native uses offset=8 which causes stride)
    {0x3810, 0x00},   // Timing horizontal offset high
    {0x3811, 0x00},   // Timing horizontal offset low
    {0x3812, 0x00},   // Timing vertical offset high
    {0x3813, 0x00},   // Timing vertical offset low
...
static const esp_cam_sensor_isp_info_t ov5647_800x640_isp_info = {
    .isp_v1_info = {
        .version = SENSOR_ISP_INFO_VERSION_DEFAULT,
        .pclk = 93312000,     // HTS x VTS x FPS = 1896 x 984 x 50
        .hts = 1896,          // Horizontal Total Size
        .vts = 984,           // Vertical Total Size
...
static const esp_cam_sensor_format_t ov5647_format_800x640_raw8_50fps = {
    ...
    .width = 800,
    .height = 640,
    .regs = ov5647_input_24M_MIPI_2lane_raw8_800x640_50fps_fixed,
    .regs_size = ARRAY_SIZE(ov5647_input_24M_MIPI_2lane_raw8_800x640_50fps_fixed),
    .fps = 50,

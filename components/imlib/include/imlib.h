...
#define IM_IS_SIGNED(a) \
    (__builtin_types_compatible_p(__typeof__(a), signed) || __builtin_types_compatible_p(__typeof__(a), signed long))
#define IM_IS_UNSIGNED(a)                                     \
    (__builtin_types_compatible_p(__typeof__(a), unsigned) || \
     __builtin_types_compatible_p(__typeof__(a), unsigned long))
#define IM_SIGN_COMPARE(a, b) ((IM_IS_SIGNED(a) && IM_IS_UNSIGNED(b)) || (IM_IS_SIGNED(b) && IM_IS_UNSIGNED(a)))
...
#define COLOR_THRESHOLD_RGB565(pixel, threshold, invert)                                     \
    ({                                                                                       \
        ...
        ((_threshold->LMin <= _l) && (_l <= _threshold->LMax) && (_threshold->AMin <= _a) && \
         (_a <= _threshold->AMax) && (_threshold->BMin <= _b) && (_b <= _threshold->BMax)) ^ \
            _invert;                                                                         \
    })
...
#define COLOR_GRAYSCALE_TO_BINARY(pixel) \
    ((pixel) > (((COLOR_GRAYSCALE_MAX - COLOR_GRAYSCALE_MIN) / 2) + COLOR_GRAYSCALE_MIN))
...
typedef enum {
    ...
    PIXFORMAT_GRAYSCALE =
        (PIXFORMAT_FLAGS_M | (PIXFORMAT_ID_GRAY << 16) | (SUBFORMAT_ID_GRAY8 << 8) | PIXFORMAT_BPP_GRAY8),
    ...
    PIXFORMAT_BAYER =
        (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) | (SUBFORMAT_ID_BGGR << 8) | PIXFORMAT_BPP_BAYER),
    PIXFORMAT_BAYER_BGGR =
        (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) | (SUBFORMAT_ID_BGGR << 8) | PIXFORMAT_BPP_BAYER),
    PIXFORMAT_BAYER_GBRG =
        (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) | (SUBFORMAT_ID_GBRG << 8) | PIXFORMAT_BPP_BAYER),
    PIXFORMAT_BAYER_GRBG =
        (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) | (SUBFORMAT_ID_GRBG << 8) | PIXFORMAT_BPP_BAYER),
    PIXFORMAT_BAYER_RGGB =
        (PIXFORMAT_FLAGS_CR | (PIXFORMAT_ID_BAYER << 16) | (SUBFORMAT_ID_RGGB << 8) | PIXFORMAT_BPP_BAYER),
    PIXFORMAT_YUV =
        (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) | (SUBFORMAT_ID_YUV422 << 8) | PIXFORMAT_BPP_YUV422),
    PIXFORMAT_YUV422 =
        (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) | (SUBFORMAT_ID_YUV422 << 8) | PIXFORMAT_BPP_YUV422),
    PIXFORMAT_YVU422 =
        (PIXFORMAT_FLAGS_CY | (PIXFORMAT_ID_YUV422 << 16) | (SUBFORMAT_ID_YVU422 << 8) | PIXFORMAT_BPP_YUV422),
...
#define IMLIB_PIXFORMAT_IS_VALID(x)                                                                                \
    ((x == PIXFORMAT_BINARY) || (x == PIXFORMAT_GRAYSCALE) || (x == PIXFORMAT_RGB565) || (x == PIXFORMAT_ARGB8) || \
     (x == PIXFORMAT_BAYER_BGGR) || (x == PIXFORMAT_BAYER_GBRG) || (x == PIXFORMAT_BAYER_GRBG) ||                  \
     (x == PIXFORMAT_BAYER_RGGB) || (x == PIXFORMAT_YUV422) || (x == PIXFORMAT_YVU422) || (x == PIXFORMAT_JPEG) || \
     (x == PIXFORMAT_PNG))
...
#define IMAGE_GET_BINARY_PIXEL(image, x, y)                                                                     \
    ({                                                                                                          \
        ...
        __typeof__(y) _y         = (y);                                                                         \
        (((uint32_t *)                                                                                          \
              _image->data)[(((_image->w + UINT32_T_MASK) >> UINT32_T_SHIFT) * _y) + (_x >> UINT32_T_SHIFT)] >> \
         (_x & UINT32_T_MASK)) &                                                                                \
            1;                                                                                                  \
    })
...
void imlib_draw_string(image_t *img, int x_off, int y_off, const char *str, int c, float scale, int x_spacing,
                       ...
                       int string_rotation, bool string_hmirror, bool string_hflip);

// void imlib_draw_char_8x16(image_t *fb, int32_t start_x, int32_t start_y, uint8_t ch, uint32_t color);
// void imlib_draw_char_16x16(image_t *fb, int32_t start_x, int32_t start_y, uint32_t code, uint32_t color);
// void imlib_draw_string(image_t *fb, uint16_t x, uint16_t y, const char *str, uint32_t color);
// void imlib_draw_pixel(image_t *fb, int x, int y, uint32_t color) ;
// void imlib_draw_line(image_t *fb, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint32_t color, uint16_t
// thickness); void imlib_draw_rectangle(image_t *fb, uint16_t rx, uint16_t ry, uint16_t rw, uint16_t rh, uint32_t
// color, uint16_t thickness, uint8_t fill); void imlib_draw_circle(image_t *fb, uint16_t cx, uint16_t cy, uint16_t
// radius, uint32_t color, uint16_t thickness, uint8_t fill);
